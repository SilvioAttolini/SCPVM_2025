function nmse=new_nmse(reference, estimate)
    % computeNMSE - Computes the Normalized Mean Squared Error (NMSE)
    %
    % INPUT:
    %   reference : Array of true/reference values
    %   estimate  : Array of estimated values
    %
    % OUTPUT:
    %   nmse_val  : Normalized Mean Squared Error (NMSE)
    
    % Ensure both arrays have the same size
    if length(reference) ~= length(estimate)
        error('The reference and estimate arrays must have the same length.');
    end
